...
describe('MeetingSessionConfiguration', () => {
  ...
  it('can take atttendee capabilities', () => {
    const configuration = new MeetingSessionConfiguration(
      {
        MeetingId: 'meeting-id',
        ExternalMeetingId: null,
    ...
    expect(configuration.attendeeCapabilities.audio).to.eq('SendReceive');
    expect(configuration.attendeeCapabilities.video).to.eq('Send');
    expect(configuration.attendeeCapabilities.content).to.eq('Receive');
  });
});
