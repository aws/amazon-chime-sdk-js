...
import ParallelGroupTask from '../../src/task/ParallelGroupTask';
import Task from '../../src/task/Task';

describe('ParallelGroupTask', () => {
  const expect: Chai.ExpectStatic = chai.expect;
  const waitTimeMs = 100;

  describe('run', () => {
    it('can be run with no subtasks', done => {
      const task: Task = new ParallelGroupTask(new NoOpLogger(), 'test', []);
      task.run().then(() => {
        done();
      });
    });
    ...
    it('can be run with one subtask', done => {
      ...
      const task: Task = new ParallelGroupTask(new NoOpLogger(), 'test', [new TestTask()]);
      task.run();
    });

    it('can run two tasks in parallel', done => {
      let currentTask = 0;
      class TestTask implements Task {
    ...
    it('waits for all tasks to finish before failing', done => {
      let taskRunCount = 0;
