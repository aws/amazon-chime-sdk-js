...
import TranscriptResult from '../../src/transcript/TranscriptResult';
...
export function makeTranscript(): Transcript {
  const startTimeMs = 1234567890123;
  const endTimeMs = startTimeMs;
  const attendee = {
    attendeeId: 'speaker-attendee-id',
    externalUserId: 'speaker-external-user-id',
  };
    ...
    alternatives: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            attendee,
            content: '.',
            type: TranscriptItemType.PUNCTUATION,
          },
        ],
      },
    ],
...
export function makeTranscriptionStatus(): TranscriptionStatus {
  const transcriptionStatus = new TranscriptionStatus();
  transcriptionStatus.type = TranscriptionStatusType.STARTED;
  transcriptionStatus.eventTimeMs = 1234567890123;
  transcriptionStatus.transcriptionRegion = 'test-region';
  transcriptionStatus.transcriptionConfiguration = 'test-configuration';
  transcriptionStatus.message = 'transcription status message';
