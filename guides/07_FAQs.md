...
- [Frequently Asked Questions](#frequently-asked-questions)
  ...
  - [Meetings](#meetings)
    ...
    - [What should my application do in response to the status codes in `audioVideoDidStop`?](#what-should-my-application-do-in-response-to-the-status-codes-in-audiovideodidstop)
...
MeetingSessionStatusCode provides additional details for the MeetingSessionStatus received for a session. While some codes are used internally for MeetingSessionStatus, their primary use is in callbacks of AudioVideoObserver. Specifically, they are utilized in `AudioVideoObserver.audioSessionDidStopWithStatus(MeetingSessionStatus)`/`AudioVideoObserver.videoSessionDidStartWithStatus(MeetingSessionStatus)`/`AudioVideoObserver.videoSessionDidStopWithStatus(MeetingSessionStatus)` for iOS, and their equivlants for Android. There are also usages in ReplicatedMeetings, they have been documented separately([iOS](https://github.com/aws/amazon-chime-sdk-ios/blob/master/guides/replicated_meetings.md)/[Android](https://github.com/aws/amazon-chime-sdk-android/blob/master/guides/replicated_meetings.md)).
...
| audioDisconnectAudio(iOS) AudioDisconnectAudio(Android)  connectionHealthReconnect(iOS)  ConnectionHealthReconnect(Android) | None | This is only used internally and will not be provided in any AudioVideoObserver callbacks. |
