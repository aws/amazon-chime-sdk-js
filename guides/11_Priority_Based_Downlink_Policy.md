...
Video might be paused or unpaused if a network event occurs or recovers. To configure the network event delays, 
refer [Configure network event delays](#configure-network-event-delays). If you want to keep the last frame of the 
video stream when pausing, please set `DefaultVideoTile.keepLastFrameWhenPause` to true so that the 
`videoElement.srcObject` is not cleared out when a video is paused. 
