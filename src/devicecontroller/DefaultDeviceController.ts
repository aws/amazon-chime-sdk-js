...
export default class DefaultDeviceController
   ...
   *
   */
  releaseAudioTransformStream(): void {
    this.logger.info('Stopping audio track for Web Audio graph');

    /* istanbul ignore next */
    const audioEnded = this.activeDevices['audio']?.endedCallback;
    for (const track of this.audioInputSourceNode.mediaStream.getTracks()) {
      track.stop();

      /* istanbul ignore else */
      if (audioEnded) {
        track.removeEventListener('ended', audioEnded);
        delete this.activeDevices['audio'].endedCallback;
  ...
  releaseVideoTransformStream(): void {
    this.chosenVideoTransformDevice.onOutputStreamDisconnect();
    this.chosenVideoTransformDevice = null;
  }

  releaseOrdinaryStream(mediaStreamToRelease: MediaStream): void {
