...
export default class DefaultSimulcastUplinkPolicy implements SimulcastUplinkPolicy {
  ...
  private publishEncodingSimulcastLayer(): void {
    const currentEncodingSimulcastLayer = this.getEncodingSimulcastLayer();
    if (this.activeSimulcastLayers === currentEncodingSimulcastLayer) {
      return;
    }
    this.activeSimulcastLayers = currentEncodingSimulcastLayer;
