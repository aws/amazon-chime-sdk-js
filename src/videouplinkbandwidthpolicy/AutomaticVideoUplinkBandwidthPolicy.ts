...
/** AutomaticVideoUplinkPolicy implements capture and encode
...
export default class AutomaticVideoUplinkPolicy implements SimulcastUplinkPolicy {
  ...
  static readonly SVCCodecNames: string[] = ['VP9', 'AV1'];
  ...
  private transceiverController: AutomaticTransceiverController;
  ...
  reset(): void {
    ...
    this.currentQualityMap = this.fillEncodingParamWithBitrates([this.maxBandwidthKbps(), 0, 0], 1);
    this.newQualityMap = this.fillEncodingParamWithBitrates([this.maxBandwidthKbps(), 0, 0], 1);
  }
  ...
  private updateOptimalParameters(): void {
    ...
    const enableSVC =
      this.uplinkTechnique === VideoUplinkTechnique.ScalableVideoCoding &&
  ...
  maxBandwidthKbps(): number {
    ...
    if (this.overrideTargetWidth && this.overrideTargetHeight) {
      const overrideRateKbps =
        (this.overrideTargetWidth * this.overrideTargetHeight * 1.6) / 1000 + 50;
  ...
  setTransceiverController(
    transceiverController: AutomaticTransceiverController | undefined
  ): void {
  ...
  private encodingParametersEqual(): boolean {
    let different = false;
    for (const ridName of AutomaticTransceiverController.NAME_ARR_DECENDING) {
      different =
        different ||
  ...
  private fillEncodingParamWithBitrates(
    ...
    const toBps = 1000;
    const nameArr = AutomaticTransceiverController.NAME_ARR_DECENDING;
    ...
    for (let i = 0; i < nameArr.length; i++) {
      ...
      newMap.set(ridName, {
        ...
        // @ts-ignore
        scalabilityMode: enableSVC && bitrateArr[i] > 0 ? 'L3T3' : 'L1T1',
      });
      scale = scale * 2;
    }
  ...
  private calculateBaseScaleFactor(setting: MediaTrackSettings): number {
    ...
    if (
      ...
      targetHeight =
        AutomaticVideoUplinkPolicy.targetHeightArray[
          Math.min(
            this.getNumberOfPublishedVideoSources(),
            AutomaticVideoUplinkPolicy.targetHeightArray.length - 1
          )
      ...
      if (this.overrideTargetWidth && this.overrideTargetHeight) {
        targetHeight = Math.min(
          targetHeight,
          Math.min(this.overrideTargetWidth, this.overrideTargetHeight)
        );
      }
  ...
  private calculateEncodingBitrates(): number[] {
    ...
    if (this.uplinkTechnique === VideoUplinkTechnique.Simulcast) {
      const shouldDisableSimulcast = this.numParticipants >= 0 && this.numParticipants <= 2;
      if (shouldDisableSimulcast) {
        ...
        this.newActiveStreams = ActiveStreams.kHiMidAndLow;
        bitrates[0].maxBitrateKbps = this.maxBandwidthKbps() * 0.7;
        bitrates[1].maxBitrateKbps = this.maxBandwidthKbps() * 0.2;
        bitrates[2].maxBitrateKbps = this.maxBandwidthKbps() * 0.1;
      } else if (this.getNumberOfPublishedVideoSources() <= 16) {
  ...
  private calculateEncodingParameters(
    ...
    if (!this.encodingParametersEqual()) {
      this.logger.info(
        `simulcast: policy:calculateEncodingParameters numSources:${this.getNumberOfPublishedVideoSources()} newQualityMap: ${this.getQualityMapString(
          this.newQualityMap
        )}`
      );
    }
  ...
  setMeetingSupportedVideoSendCodecs(
    ...
    for (const codecPreference of codecPreferences) {
      if (!degradedVideoSendCodecs.some(degradedCodec => codecPreference.equals(degradedCodec))) {
        codecs.push(codecPreference);
      }
    }

    const isUsingSVCCodec =
      codecs.length > 0 && AutomaticVideoUplinkPolicy.SVCCodecNames.includes(codecs[0].codecName);
