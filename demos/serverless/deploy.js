  ...
  while (i < args.length) {
    switch(args[i]) {
      ...
      case '-r': case '--region':
        ...
        break;
      case '-u': case '--use-chime-meetings-sdk':
        useChimeMeetingsSDK = getArgOrExit(++i, args)
        break;
...
console.log(`Using region ${region}, useChimeMeetingsSDK ${useChimeMeetingsSDK}, bucket ${bucket}, stack ${stack}, endpoint ${chimeEndpoint}, enable-termination-protection ${enableTerminationProtection}, disable-printing-logs ${disablePrintingLogs}`);
ensureBucket();
...
let parameterOverrides = `Region=${region} UseChimeMeetingsSDK=${useChimeMeetingsSDK} UseEventBridge=${useEventBridge} ChimeEndpoint=${chimeEndpoint} ChimeServicePrincipal=${chimeServicePrincipal}`
