...
function usage() {
  ...
  console.log(`  -u, --use-regional                   Flag to switch between chime and regional client, default '${useRegional}'`);
  ...
  while (i < args.length) {
    switch(args[i]) {
      ...
      case '-r': case '--region':
        ...
        break;
      case '-u': case '--use-regional':
        useRegional = getArgOrExit(++i, args)
        break;
...
console.log(`Using region ${region}, useRegional ${useRegional}, bucket ${bucket}, stack ${stack}, endpoint ${chimeEndpoint}, enable-termination-protection ${enableTerminationProtection}, disable-printing-logs ${disablePrintingLogs}`);
ensureBucket();
...
let parameterOverrides = `Region=${region} UseRegional=${useRegional} UseEventBridge=${useEventBridge} ChimeEndpoint=${chimeEndpoint} ChimeServicePrincipal=${chimeServicePrincipal}`
